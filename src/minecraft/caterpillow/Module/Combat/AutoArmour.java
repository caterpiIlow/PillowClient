...
import caterpillow.utils.ArmourUtils;
...
public class AutoArmour extends Module {
	...
	public AutoArmour() {
		super("AutoArmour", 0, Category.MISC);
	}
		...
		if (delay >= 10) {
			if (mc.thePlayer.inventory.armorInventory[0] == null) {
				...
				for (int i = 0; i < length; i++) {
					int id = boots[i];
					if (ArmourUtils.getItem(id) != -1) {
						item = ArmourUtils.getItem(id);
						break;
					}
				}
			}
	...
	public void betterArmor() {
		...
		if (delay >= 10 && (mc.thePlayer.openContainer == null || mc.thePlayer.openContainer.windowId == 0)) {
			...
			if (ArmourUtils.isBetterArmor(0, this.boots)) {
				item = 8;
				switchArmor = true;
			}
			if (mc.thePlayer.inventory.armorInventory[3] == null) {
			...
			if (ArmourUtils.isBetterArmor(3, this.helmet)) {
				item = 5;
				switchArmor = true;
			}
			if (mc.thePlayer.inventory.armorInventory[1] == null) {
			...
			if (ArmourUtils.isBetterArmor(1, this.leggings)) {
				item = 7;
				switchArmor = true;
			}
			if (mc.thePlayer.inventory.armorInventory[2] == null) {
			...
			if (ArmourUtils.isBetterArmor(2, this.chestplate)) {
				item = 6;
				switchArmor = true;
			}
			boolean b = false;
