...
public class Potions extends Module {
	...
	public static class DummyComparator implements Comparator<String> {

		public int compare(String potion1, String potion2) {
			if (Minecraft.getMinecraft().fontRendererObj
					.getStringWidth(potion1) > Minecraft.getMinecraft().fontRendererObj.getStringWidth(potion2))
				return -1;
			else
				return 1;
		}
	...
	public void renderDummy(ScreenPosition pos) {
		...
		potionNames.add("Stupidity I 5:00");
		potionNames.add("Itchy II 0:59");
		potionNames.add("Hype III 2:43");
		potionNames.add("IQ Boost M 2:12");

		Collections.sort(potionNames, new DummyComparator());
		for (String p : potionNames) {

			String potionName = p;
			fr.drawStringWithShadow(potionName, pos.getAbsoluteX() + 2,
					(int) count * (fr.FONT_HEIGHT + 2) + pos.getAbsoluteY() + 2, rainbow(count * 10D, 10D));
			count++;
		}
	}
