...
public class ClickGUI extends GuiScreen {
	 ...
	 * eigenen Codes hier Client.startClient() das GUI wird dann so ge ffnet:
	 * mc.displayGuiScreen(Client.clickgui); this.setToggled(false); das Module wird
	 * sofort wieder beendet damit n chstes mal nicht 2mal der z.B. 'RSHIFT' Knopf
	 * gedr ckt werden muss
	 */
	public ClickGUI() {
		 ...
		 * panels, was mit 'Collections.reverse(rpanels);' nicht ganz funktionieren w
		 * rde. Und da die Elemente nur 'r berkopiert' werden gibt es keine Probleme ;)
		 */
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		 ...
		 * manchmal egal w re ob panels/rpanels benutzt wird habe ich mich einfach mal
		...
		mb = null;
		/*
		 * berpr fen ob ein Button listening == true hat, wenn ja, dann soll nicht mehr
				...
				for (ModuleButton b : panel.Elements) {
					if (b.extended && b.menuelements != null && !b.menuelements.isEmpty()) {
						...
						for (Element e : b.menuelements) {
							e.offset = off;
							e.update();
							if (Client.instance.settingsManager.getSettingByName("Design").getValString()
		...
		if (mb != null) {
			...
			FontUtil.drawTotalCenteredStringWithShadow(
					"Press 'ESCAPE' to unbind " + mb.mod.getName()
							+ (mb.mod.getKey() > -1 ? " (" + Keyboard.getKeyName(mb.mod.getKey()) + ")" : ""),
					0, 0, 0xffffffff);
		...
		/*
		 * Nicht ben tigt, aber es ist so einfach sauberer ;) Und ohne diesen call k
		 * nnen keine GUIButtons/andere Elemente gerendert werden
		 */
	...
	public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
		...
		/*
		 * Ben tigt damit auch mit Elements interagiert werden kann besonders zu
		 * beachten ist dabei, dass zum einen rpanels aufgerufen wird welche eine
		 * Eigenst ndige Kopie von panels ist, genauer oben erkl rt Also rpanels damit
		 * zuerst das panel 'untersucht' wird, dass als letztes gerendert wurde => Ganz
		 * oben ist! sodass der Nutzer nicht mit dem Unteren interagiern kann, weil er
		 * es wohl nicht will. Und damit nicht einfach mit Panels anstatt Elements
		 * interagiert wird werden hier nur die Settings untersucht. Und wenn wirklich
		 * interagiert wurde, dann endet diese Methode hier. Das ist auch in anderen
		 * Loops zu beobachten
		 */
		...
		/*
		 * Ben tigt damit mit ModuleButtons interagiert werden kann und Panels
		 * 'gegriffen' werden k nnen
		 */
		for (Panel p : rpanels) {
	...
	public void mouseReleased(int mouseX, int mouseY, int state) {
		...
		/*
		 * Eigentlich nur f r die Slider ben tigt, aber durch diesen Call erf hrt jedes
		...
		/*
		 * Ben tigt damit Slider auch losgelassen werden k nnen und nicht immer an der
		 * Maus 'festkleben' :>
		 */
		for (Panel p : rpanels) {
	...
	public void onGuiClosed() {
		...
		Client.instance.moduleManager.getModulesByName("GUI").toggle();
